...
from model.layer.pooling import AveragePooling, MaxPooling, KMaxPooling
...
class ConvLayer(nn.Module):
    ...
    def __init__(self, in_dim, kernel_num=128, kernel_sizes=[3,4,5], activation='ReLU', use_bn=True, pool_method='max', keep_seq_length=False, **kwargs):
        """
        ...
        if self.keep_seq_length:
            for k in self.kernel_sizes:
                if k % 2 == 0:
                    raise ValueError(f"Expect all odd kenel sizes when keep_seq_length is True, but got {k}")   
        # layers
        self.kernels = nn.ParameterList()
        self.batch_norms = nn.ModuleList() if use_bn else None
        for i in range(len(kernel_sizes)):
            self.kernels.append(nn.Parameter(torch.randn(kernel_num, 1, kernel_sizes[i], in_dim, requires_grad=True, dtype=torch.float32)))
            if use_bn:
    ...
    def forward(self, x, length_index=None):
        ...
        for i, kernel in enumerate(self.kernels):
            ...
            o_i = F.conv2d(x, kernel, bias=None, padding=pad_num) # [B, Kn, L, 1] if keep_seq_length else [B, Kn, L-ks+1, 1]
        ...
        o = torch.cat(o_all, 1) if len(o_all)>1 else o_all[0] # [B, Kn*#ks, L] if pool is None else [B, k*Kn*#ks]

        # print(o.shape, length_index.shape)
        if self.keep_seq_length:
            o = o.transpose(1, 2).contiguous() * length_index.unsqueeze(-1) # [B, L, Kn*#ks]
...
class GRULayer(nn.Module):
    ...
    def __init__(self, in_dim, h_dim=128, layer_num=1, bidirectional=False, train_init_step=False, dropout=0.):
        ...
        self.h_dim = h_dim
        # layers
        self.rnn = nn.GRU(in_dim, self.h_dim, self.layer_num, bidirectional=bidirectional, batch_first=True, dropout=dropout)
        # params
        if self.train_h0:
            self.h0 = nn.Parameter(torch.empty(self.direction_num * self.layer_num, 1, self.h_dim), requires_grad=True)
            self.init_weight()
    ...
    def forward(self, x, length_index):
        ...
        x = nn.utils.rnn.pack_padded_sequence(x, lens_sort, batch_first=True, enforce_sorted=True)
        # continuous memory chunk for cudnn usage
        self.rnn.flatten_parameters()
        o, h_n = self.rnn(x, self.h0.repeat(1, batch_size, 1)) if self.train_h0 else self.rnn(x) # [L, B, Dn*H], [Ln*Dn, B, H]
        # pad packed sequence
        o = nn.utils.rnn.pad_packed_sequence(o, batch_first=True, total_length=len_max)[0] # [B, L, Dn*H]
        # concat all directions' hidden state of last step in last layer
        h_n = h_n.transpose(0, 1).contiguous().view(batch_size, self.layer_num, self.direction_num, self.h_dim) # [B, Ln, Dn, H]
...
class LSTMLayer(nn.Module):
    ...
    def __init__(self, in_dim, h_dim=128, layer_num=1, bidirectional=False, train_init_step=False, dropout=0.):
        ...
        # layers
        if layer_num > 1:
            self.rnn = nn.LSTM(in_dim, self.h_dim, self.layer_num, bidirectional=bidirectional, batch_first=True, dropout=dropout)
        else:
            self.rnn = nn.LSTM(in_dim, self.h_dim, self.layer_num, bidirectional=bidirectional, batch_first=True, dropout=0)

        # params
        if self.train_h0:
            self.h0 = nn.Parameter(torch.empty(self.direction_num * self.layer_num, 1, self.h_dim), requires_grad=True)
            self.c0 = nn.Parameter(torch.empty(self.direction_num * self.layer_num, 1, self.h_dim), requires_grad=True)
            self.init_weight()
    ...
    def forward(self, x, length_index):
        ...
        # continuous memory chunk for cudnn usage
        self.rnn.flatten_parameters()
        o, h_c_n = self.rnn(x, (self.h0.repeat(1, batch_size, 1), self.c0.repeat(1, batch_size, 1))) if self.train_h0 else self.rnn(x) # [L, B, Dn*H], tupel([Ln*Dn, B, H], [Ln*Dn, B, H])
        # pad packed sequence
        o = nn.utils.rnn.pad_packed_sequence(o, batch_first=True, total_length=len_max)[0] # [B, L, Dn*H]
        # concat all directions' hidden state of last step in last layer
        h_n = h_c_n[0].view(self.layer_num, self.direction_num, batch_size, self.h_dim)[-1,:,:,:].transpose(0,1).contiguous().view(batch_size, -1) # [B, Dn*H]
        # re-sort by lens
        o = o.index_select(0, idx_ori) # [[[1f,1b][2f,2b]...[max_Lenf,max_Lenb]]] used in sequences tagging
