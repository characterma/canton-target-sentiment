...
locust.stats.CSV_STATS_FLUSH_INTERVAL_SEC = (
    10
)  # Determines how often the datais flushed to disk, default is 10 seconds

# api setting
HOST = "http://0.0.0.0:8080"
URL = "/target_sentiment"
TEST_INPUT = {
    "content": ("## Headline ##\n大家最近買左D乜 分享下?\n## Content ##\n引用:\n原帖由 綠茶寶寶 於 2018-12-31 08:40 PM" 
...
class Testlen(TaskSet):
    @task
    def test(self):
        self.client.post(url=URL, json=TEST_INPUT)
...
def runTest(u, r, file, t):
    ...
    cmd = "locust -f locustfile.py --host={} --headless --csv={} --csv-full-history -u{} -r {} -t {}".format(
        HOST, file, u, r, t
    )
