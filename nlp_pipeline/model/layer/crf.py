...
class LinearChainCRF(nn.Module):
    def __init__(self, in_dim, num_tags):
        ...
        self.linear_projection = (
            nn.Linear(in_dim, num_tags) if in_dim != num_tags else None
        )
        self.transition_matrix = nn.Parameter(
            torch.FloatTensor(self.Nc_se, self.Nc_se), requires_grad=True
        )  # [i,j] means score from i to j
        self.register_buffer(
            "start_end_state_emit_score",
            torch.full((1, 1, 2), self.initial_score, requires_grad=False),
        )
        self.register_buffer(
            "start_step_emit_score",
            torch.full((1, self.Nc_se), self.initial_score, requires_grad=False),
        )
    ...
    def forward(self, x, length_index=None):
        B, L_max, H = x.size()
        mask = (
            torch.ones(B, L_max, dtype=torch.bool, device=x.device)
            if length_index is None
            else length_index.view(B, L_max).bool()
        )
        ...
        x = torch.cat(
            [x, self.start_end_state_emit_score.expand(B, L_max, -1)], dim=-1
        )  # [B, L_max, Nc_se]
        return x, mask
    ...
    def nll_loss(self, x, y, length_index=None, reduce="mean"):
        ...
        """
        x, mask = self.forward(
            x, length_index=length_index
        )  # [B, L_max, Nc_se], [B, L_max]
        ...
        if reduce == None:
            ...
            raise ValueError(
                f"Expect `reduce` is one of ['mean', 'sum'], but got {reduce}"
            )
    ...
    def _forward_alg(self, x, mask):
        ...
        for t in range(0, L_max):
            # [B, Nc_se, Nc_se]
            alpha = (
                alpha_previous.view(B, Nc_se, 1)
                + self.transition_matrix.view(1, Nc_se, Nc_se)
                + x[t].view(B, 1, Nc_se)
            )
            alpha_previous = torch.logsumexp(alpha, dim=1) * mask[t].view(
                B, 1
            ) + alpha_previous * (~mask[t]).view(
                B, 1
            )  # [B, Nc_se]
        # Termination: at step END, score = last + trans_to_END
        all_path_score = torch.logsumexp(
            alpha_previous + self.transition_matrix[:, self.end_idx].view(1, Nc_se),
            dim=-1,
        )  # [B]
    ...
    def _path_score(self, x, y, mask):
        ...
        path_score = self.transition_matrix[self.start_idx, :].index_select(
            0, y[0]
        )  # [B]
        # from step 0 to step L_max - 2
        ...
        real_last_tag = y.gather(0, (mask.sum(0).long() - 1).view(1, B)).squeeze(
            0
        )  # [B]
        emit_score_last_tag = (
            x[-1].gather(1, real_last_tag.view(B, 1)).squeeze(1) * mask[-1]
        )  # [B]
        # transition score from real last tag to END (not the pad to END)
        trans_score_to_end = self.transition_matrix[:, self.end_idx].index_select(
            0, real_last_tag
        )
    ...
    def viterbi_decode(self, x, length_index=None, top_k=1):
        ...
        for ins, ins_mask in zip(x, mask):  # [L_max, Nc_se], [L_max]
            ...
            v_stop = (
                v_previous.unsqueeze(2)
                + self.transition_matrix[:, self.end_idx].view(-1, 1)
            ).view(-1)
